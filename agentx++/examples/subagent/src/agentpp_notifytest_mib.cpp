...
static const char *loggerModuleName = "agentx++.agentpp_notifytest_mib";
...
void agentppNotifyTest::get_request(Request* req, int ind)
{

	//--AgentGen BEGIN=agentppNotifyTest::get_request
	//--AgentGen END
...
void agentppNotifyTest::send_agentppNotifyTestAllTypes() {
	...
	OpaqueStr op(s);
	vbs[n++].set_value(op);
	agentppNotifyTestAllTypes no((SubAgentXMib*)SubAgentXMib::instance);
	no.generate(vbs, 9, "");
	for (int j=0; j<9; j++) {
...
agentppNotifyTestAllTypes::agentppNotifyTestAllTypes(SubAgentXMib* mib)
{
	my_mib = mib;
...
void agentppNotifyTestAllTypes::generate(Vbx* vbs, int sz, const OctetStr& context)
{
	...
	my_mib->notify(context, oidAgentppNotifyTestAllTypes, vbs, sz);
}
...
agentpp_notifytest_mib::agentpp_notifytest_mib(const OctetStr& context, SubAgentXMib* mib):
